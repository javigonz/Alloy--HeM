...
exports.createComboBox = function() {};
...
exports.strNormalize = function(str) {
    ...
    pares.str2 = "¡";
    caracteres.push(pares);
    for (var i = 0; i <= caracteres.length - 1; i++) str = str.split(caracteres[i].str1).join(caracteres[i].str2);
    return str;
};
...
exports.removeAllRows = function(table) {
    if (table.data.length > 0) for (var i = table.data[0].rows.length - 1; i >= 0; i--) table.deleteRow(i);
};
...
exports.httpRequest = function(method, url, payload, callback) {
    var client = Ti.Network.createHTTPClient({
        onload: function() {
            callback && callback(this.responseText, null);
        },
        onerror: function(e) {
            "404" == e.error || "Not Found" == e.error ? callback(JSON.stringify({
                list: []
            }), null) : callback && callback(this.responseText, e.error);
        },
        timeout: 5e3
    });
    ...
    ("POST" == method || "PUT" == method) && client.setRequestHeader("Content-Type", "application/json");
    client.send(null != payload ? JSON.stringify(payload) : payload);
};